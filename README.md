...
bilbo 는 Linux, macOS, Windows 에서 사용 가능하며, Python 3.6 이상 버전을 필요로 한다. 또한 AWS 를 기반으로 하기에, AWS 가입 및 기본적인 대쉬보드 이용은 가능한 것으로 가정하겠다.
...
Dask 워커를 띄울 때, 아래와 같이 옵션으로 몇 개의 프로세스와 스레드를 사용할지와, 메모리 제약을 줄 수 있다.
...
작업의 특성에 맞게 아래와 같이 커스텀한 값을 설정할 수 있다.
...
이렇게 하면 스레드를 4개 가진 워커 프로세스 하나가 전체 시스템의 메모리를 다 사용하게 된다.